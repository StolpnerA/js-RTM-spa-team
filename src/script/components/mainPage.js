...
class mainPage {
  printMessages(userInfo, message, placeMsg) {
    ...
    let tplName = isMyMessage
      ? "myMsg"
      : "opponentMsg";
    let tpl = readTemplate(tplName);
    let idUser = isMyMessage
  ...
  render() {
    ...
    place.innerHTML = document
      .getElementById("mainChat")
      .innerHTML;
  }
  ...
  userInfo() {
    ...
    slackApi
      .userInfo(token, user)
      .then(data => {
        $$(".userName").innerHTML = `<img src="${data.user.profile.image_32}"> ${data.user.name}`;
      });
  }
            ...
            do {
              ...
              if (text.indexOf("&lt;map&gt;") == 0) {
                text = text
                  ...
                  .split(",");
                text.push(leng);
                let div = `<div id="mapSend${ `${leng}`.replace('.', '')}" style="width: 100%; height: 200px"></div>`;
                let tpl = document
                ...
                if (localStorage.getItem("user") == data.messages[leng].user) {
                  ...
                  if (data.messages[leng].file != undefined && data.messages[leng].file.thumb_360 != undefined) {
  ...
  replaceMsg(text, count, name, img, placeMsg, sound) {
    ...
    if (text.indexOf("&lt;map&gt;") == 0) {
      ...
      let div = `<div id="mapSend${ `${count}`.replace('.', '')}" style="width: 100%; height: 200px"></div>`;
  ...
  renderMsg(message, token) {
    ...
    if (message.ts == "1501544614.596385" || message.ts == "1501796123.395835") {
      return;
    }
    let placeMsg = $$(".workPlace");
    let sound = $$("#audio");
    ...
    slackApi
      ...
      .then(data => {
        ...
        let tplName = isMyMessage
          ? "myMsg"
          : "opponentMsg";
        if (tplName == "opponentMsg") 
          sound.play();
        let tpl = readTemplate(tplName);
        text = sendImg && sendImg.thumb_360
  ...
  removeChannel(token, className) {
    ...
    slackApi
      ...
      .then(slackApi.channelLeave(token, className).then(localStorage.setItem("channel", "C6CS9BNG3")) // есть косяк, в других чатах не будет перезходить на general
        .then(this.loadhistoryMessage()));
  }
  ...
  GetYandexMap() {
    ...
    function init() {
      ...
      $$("#map").addEventListener("click", ev => {
        if (!ev.target.matches(".sendCoords")) 
          return;
        ...
        .add("click", e => {
          if (!myMap.balloon.isOpen()) {
            ...
            myMap
              ...
              .open(coords, {
                contentHeader: "Event",
                contentBody: "<p>Data to send.</p><p>This coordinates: " + [
      ...
      myMap
        ...
        .add("contextmenu", function (e) {
          myMap
            .hint
            .open(e.get("coords"), "Someone right-clicked");
        });
      myMap
        .events
